    ...
    if (messageType === "text") {
        ...
        try {
            ...
            if (mainMenuItems.length === 0) {
                ...
                const actionMenuNames = await handleAction(mainMenuItems[0].ACTION.split("~"), clientId, mainMenuItems[0].MENU_ID, title, selectId, from, Appointment_ID, userData, branchID);
                ...
                if (actionMenuNames !== null) {
                    ...
                    if (actionMenuNames[0].Appointment_ID) {
                        menuNames = actionMenuNames.map((item) => ({ id: item.CLIENT_ID + "~" + item.MENU_ID + "~" + item.ITEM_ID + "~" + item.Appointment_ID + "~" + item.branchID + "|" + clientId + "~" + menuId + "~" + selectId + "~" + branchID, title: item.MENU_NAME, }));
                    } else {
                        menuNames = actionMenuNames.map((item) => ({ id: item.CLIENT_ID + "~" + item.MENU_ID + "~" + item.ITEM_ID + "~" + Appointment_ID + "~" + item.branchID + "|" + clientId + "~" + menuId + "~" + selectId + "~" + branchID, title: item.MENU_NAME, }));
                    }
                ...
                const menuNames = mainMenuItems.map((item) => ({ id: item.CLIENT_ID + "~" + item.MENU_ID + "~" + item.ITEM_ID + "~" + Appointment_ID + "~" + item.branchID + "|" + clientId + "~" + menuId + "~" + selectId + "~" + branchID, title: item.MENU_NAME, }));
                await sendRadioButtonMessage(from, headerMessage, menuNames);
                if (clientJsonData["backButton"]) {
                    await sendBackButton(from, previousId, Appointment_ID, mainMenuItems);
                }
...
async function handleAction(iAction, iClientId, iMenuId, iUserValue, iSelectId, from, Appointment_ID, userData, branchID) {
    const iLang = "ENG";
    ...
    if (iUserValue != "Back" && iUserValue != "Cancel Appointment" && iUserValue != "Reschedule") {
        await updateAppointment(iAction[0].split("~")[0], iUserValue, Appointment_ID, iSelectId, branchID);
        logger.info("Appointment updated successfully");
    }

    if (iAction[1] === "LIST") {
        return await getFromList(iClientId, iMenuId, iAction[2], iLang, Appointment_ID);
    } else if (iAction[1] === "POC") {
        return await getPocFromPoc(iClientId, iMenuId, await getAppointmentJsonDataByKey(Appointment_ID, "Department"), Appointment_ID);
    ...
    if (iAction[1] === "CONFIRM") {
        ...
        if (iUserValue === "Confirm") {
            ...
            const intervalId = setInterval(async() => {
                ...
                if (paymentStatus === 'paid') {
                    // Payment successful - automatically finalize
                    ...
                    // Finalize the appointment automatically
                    ...
                    // Send POC notification if needed
                    ...
                    // ✅ Only send countdown reminder if time is 9, 6, or 3 mins left
                    if ([9, 6, 3].includes(minutesLeft)) {
                        await sendWhatsAppMessage(
                            from,
            ...
            }, process.env.PAYMENT_CHECK_INTERVAL || 60000); // Still checking every 60s
            // Check and notify every 60 seconds
        ...
        jsonData["Appointment_Date"] = appointmentDetails[0].Appointment_Date;
        jsonData["Appointment_Time"] = appointmentDetails[0].Appointment_Time;
        jsonData["Payment_Status"] = appointmentDetails[0].Payment_Status;
        const transaction_id = await getTransactionIdByAppointmentId(Appointment_ID);
        ...
        await updateAppointment("Payment_Status", jsonData["Payment_Status"], Appointment_ID);
        await updateAppointmentJsonData(Appointment_ID, "Payment_Status", jsonData["Payment_Status"]);
        await updateAppointmentIdByTransactionId(transaction_id, Appointment_ID);
...
async function finalizeAppointment(iClientId, iAction, Appointment_ID, userData) {
    ...
    if (!paymentStatus) {
        await updateAppointment("Payment_Status", "Pay_later", Appointment_ID);
    }
